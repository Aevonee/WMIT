...
function inquiryAiPrefillCard() {
  return '<div class="card"><details class="secondary-details"><summary>Paste client message (AI pre-fill)</summary><p class="muted">AI pre-fill is a draft: it only fills empty fields in the Create Inquiry form — the inquiry is only created when you save the form.</p><div class="field"><label>Client message (email or Viber text)</label><textarea id="inq-ai-text" rows="5" maxlength="8000" oninput="inquiryAiText=this.value" placeholder="Paste the client email or chat text here">' + esc(inquiryAiText) + '</textarea></div><button class="secondary" onclick="parseClientMessage(this)">Parse and pre-fill</button></details></div>';
}

async function parseClientMessage(button) {
  const textarea = $('inq-ai-text');
  if (!textarea) return;
  inquiryAiText = textarea.value;
  if (!inquiryAiText.trim()) return failLocal('Paste the client message text first.', 'inq-ai-text');
  if (button && button.disabled !== undefined) button.disabled = true;
  let result = null;
  try {
    result = await api('parseInquiryMessage', { text: inquiryAiText }, 'LOCAL_STAFF');
  } finally {
    if (button && button.disabled !== undefined) button.disabled = false;
  }
  if (!result) return;
  if (result.parse_available === false) return failLocal(result.message || 'AI inquiry parsing is unavailable. Fill the form manually from the client message.', 'inq-ai-text');
  const mapping = { destination: 'inq-destination', travel_start: 'inq-start', travel_end: 'inq-end', travel_month: 'inq-month', travel_year: 'inq-year', duration_days: 'inq-duration-days', adults: 'inq-adults', children: 'inq-children', infants: 'inq-infants' };
  let filled = 0;
  Object.keys(mapping).forEach((key) => {
    const value = result.fields && result.fields[key];
