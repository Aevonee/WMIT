...
Phase 3 — hosted-server foundation complete and expo preparation. The SQLite-backed hosted server (`npm start`, `src/server/`) is the deployment target: session authentication, hash-chained audit, scheduler with nightly verified backups, and staging/production environments. Google Workspace remains unavailable and must not be accessed or configured; `apps-script/` is a retained working artifact, not the target. The immediate business deadline is the September 4–6 expo; expo lead capture, quote delivery, and follow-up tooling take priority over further architecture work until it ships.
